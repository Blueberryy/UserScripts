// ==UserScript==
// @name              Greasyfork Display registration time
...
// @name:ru           Greasyfork: кнопка времени регистрации
...
// @description:ru    Если ваш скрипт затрагивает интересы определённых лиц, получение вредоносных негативных отзывов неудивительно. Вредоносные негативные отзывы имеют две особенности: во-первых, учётная запись этих пользователей обычно недавно зарегистрирована, а во-вторых, отзыв даётся вскоре после регистрации и практически без последующей активности. Этот скрипт получает время регистрации пользователя GreasyFork и отображает его рядом с его именем пользователя. Если время, отображаемое рядом с именем пользователя, находится в будущем, то, скорее всего, этот пользователь является злонамеренно зарегистрированной учётной записью.
